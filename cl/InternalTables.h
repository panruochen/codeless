...
struct ReadReq {
	const char *line; /* in:     Optional - The text line */
	const char *cp;   /* in/out: Current position of text to be processed */
	CC_STRING error;  /* out:    The error messages */
	SynToken  token;  /* out:    The read and returned token */

	inline ReadReq() {
		this->line = this->cp = NULL;
	}
	inline ReadReq(const char *line) {
		this->line = this->cp = line;
	}
};
